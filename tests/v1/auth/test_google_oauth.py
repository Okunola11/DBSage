...
def mock_google_oauth2(monkeypatch):
    ...
    monkeypatch.setattr(
        google_oauth.google, "authorize_redirect", mock_authorize_redirect
    )
    monkeypatch.setattr(
        google_oauth.google, "authorize_access_token", mock_authorize_token_userinfo
    )
    monkeypatch.setattr(
        GoogleOAuthService,
        "get_callback_response",
        lambda self, *args: generate_token_return,
    )
