...
class GoogleOAuthService(Service):
        ...
        try:
            ...
            if existing_user:
                ...
                try:
                    ...
                    print(
                        f"GOOGLE ACCESS TOKEN IS {google_response.get('access_token')}"
                    )
                    # oauth data
                    oauth_data = OAuth(
                        user_id=new_user.id,
    ...
    def get_response(
        self, user: object, db: Annotated[Session, Depends(get_db)]
    ) -> RedirectResponse:
        ...
        try:
            ...
            user_session = {
                ...
                "token_expiry": int(datetime.now().timestamp() * 1000)
                + ((tokens.get("expires_in") or 0) * 1000),
            }
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Exception occured in get_response method; {exc}",
            )
