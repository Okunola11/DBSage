...
from db_sage.app.core.config.security import (
    encrypt_string,
    decrypt_string,
)
...
class GoogleOAuthService(Service):
    ...
    def create(
        ...
        try:
            ...
            existing_user = (
                db.query(User).filter_by(email=user_info.get("email")).one_or_none()
            )

            if existing_user:
                # retrieve the user's google_access_token
                oauth_data = (
                    db.query(OAuth).filter_by(user_id=existing_user.id).one_or_none()
                )
                # if it exists
                if oauth_data:
                    self.update(oauth_data, google_response, db)
                    # pass the existing user to the get_callback_response method to generate a response object
                    return self.get_callback_response(
                        user=existing_user,
                    ...
                    try:
                        ...
                        db.commit()
                        # pass the user object to get_callback_response method to generate a response object
                        return self.get_callback_response(
                            existing_user, google_response.get("id_token"), db
                        )
                    except Exception as exc:
                        db.rollback()
                        raise HTTPException(
                            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                            detail=f"Exception occured in oauth creation; {exc}",
                        )
            else:
                try:
                    ...
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail=f"Exception occured in user creation; {exc}",
                    )

                db.refresh(new_user)

                # pass the user object to the get_callback_response model to get a response
                return self.get_callback_response(
                    new_user, google_response.get("id_token"), db
                )
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Exception occured in create method; {exc}",
            )
    ...
    def update(
        ...
        try:
            ...
            oauth_data.id_token = google_response.get("id_token")
            oauth_data.updated_at = datetime.now(timezone.utc)
            oauth_data.expires_at = datetime.now(timezone.utc) + timedelta(minutes=5)
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Exception occured in update method; {exc}",
            )

    def get_callback_response(
        self, user: User, id_token: str, db: Session
    ) -> RedirectResponse:
        """Returns the response for the callback route"""

        try:
            # Create a unique token for user verification
            token = self.get_google_secure_token(db, id_token, user.id)

            secured_token = encrypt_string(token)
            # Send a redirect to the oauth callback page of the frontend
            return RedirectResponse(
                url=f"{settings.FRONTEND_URL}/auth/google-callback?token={secured_token.decode()}"
            )
        except Exception as exc:
            raise HTTPException(
                status_code=400,
                detail=f"Exception in get_callback_respone method, {exc}",
            )

    def get_google_secure_token(self, db: Session, id_token: str, user_id: str):
        """Generates unique token"""

        context = "GOOGLE OAUTH"
        return f"{context}:{id_token}:{user_id}".strip()

    def generate_oauth_login_tokens(self, data: GoogleLoginTokenRequest, db: Session):
        """Generate login tokens for google oauth"""

        token = data.token
        decrypted_token = decrypt_string(token.encode())

        context = decrypted_token.split(":")
        if len(context) != 3:
            raise HTTPException(status_code=400, detail="Invalid token")
        id_token = context[1]
        user_id = context[2]

        secure_token = self.get_google_secure_token(db, id_token, user_id)

        if secure_token != decrypted_token:
            raise HTTPException(status_code=400, detail="Invalid token")

        oauth_data = (
            db.query(OAuth).filter_by(user_id=user_id, id_token=id_token).one_or_none()
        )

        if not oauth_data:
            raise HTTPException(status_code=400, detail="Invalid request!")

        if datetime.now(timezone.utc) > oauth_data.expires_at:
        ...
        pydantic_model = GoogleLoginTokenResponse(
