...
from db_sage.app.utils.email_context import FORGOT_PASSWORD, USER_VERIFY_ACCOUNT
from db_sage.app.utils.settings import settings
from db_sage.app.utils.logger import logger
from db_sage.app.utils.string import unique_string
from db_sage.app.utils.db_validators import check_model_existence
...
from db_sage.app.v1.services.email import (
...
from db_sage.app.v1.responses.user import (
    ...
    SuperAdminFetchUserResponse,
    SuperAdminUserResponseData,
...
class UserService(Service):
    ...
    def fetch(self, db: Session, id):
        ...
        response = SuperAdminFetchUserResponse(
            message="Successfully fetched user", data=user_data
        )
        return response
    ...
    def fetch_all(
        ...
        users = (
            db.query(User).filter(*filters).limit(per_page).offset(offset_value).all()
        )

        total_users = len(users)
        if total_users:
            all_user_data = [
                SuperAdminUserResponseData.model_validate(user, from_attributes=True)
                for user in users
            ]
            ...
            return response
        response = FetchAllUsersResponse(
            message="No User(s) found", page=page, per_page=per_page, total=0, data=[]
        )
        return response

    def update(
        self, db: Session, current_user: User, data, id: Annotated[str, Optional] = None
    ):
        ...
        if (not id) or (id and current_user.id == id) or current_user.is_superadmin:
            ...
            if user.is_deleted:
                raise HTTPException(
                    status_code=400, detail="User is deleted and cannot be updated"
                )

            if not user.is_verified:
                raise HTTPException(
                    status_code=400, detail="User is not verified and cannot be updated"
                )

            if not user.is_active:
                raise HTTPException(
                    status_code=400, detail="User is inactive and cannot be updated"
                )
    ...
    async def activate_user_account(self, data, db: Session, background_tasks):
        ...
        if not token_valid:
            raise HTTPException(
                status_code=400, detail="This link is either expired or not valid"
            )
    ...
    async def get_login_token(self, data, db: Session):
        ...
        existing_token = (
            db.query(UserToken)
            .filter(
                UserToken.user_id == user.id, UserToken.expires_at > datetime.utcnow()
            )
            .first()
        )
    ...
    def _generate_tokens(self, user: User, db: Session):
        ...
        refresh_token = generate_token(
            refresh_token_payload, settings.SECRET_KEY, settings.ALGORITHM, rt_expires
        )
    ...
    async def email_forgot_password_link(self, data, background_tasks, db):
        ...
        return success_response(
            status_code=200, message="Please check your mail to change password"
        )
    ...
    async def reset_user_password(self, data, db: Session):
        ...
        return success_response(
            status_code=200, message="Your password has been updated."
        )
